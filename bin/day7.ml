...
let parse_directory (pwd: dir_node) line: dir_node = 
  ...
  | ["dir"; dir_name] -> pwd.dirs <- ({name=dir_name; files=[]; dirs = []; parent = Some(pwd)} :: pwd.dirs); pwd
...
let rec compute_directory_sizes ?sizes node =
  ...
  if Option.is_some (Hashtbl.find init_sizes (key_of_node node)) then 
    ...
    Hashtbl.add_exn new_sizes ~key:(key_of_node node)
      ~data:folder_size;
...
let () =
  ...
  let large_enough_dirs = Hashtbl.filter sizes ~f: (fun folder_size -> folder_size > to_delete) in
