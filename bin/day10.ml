...
let value_at_cycle trace cycle = 
  let (_, value) = List.find_exn trace ~f:(fun (cycle_count, _) -> cycle_count <= cycle) in
  value

let compute_signal_strength (trace: (int * cpu_state) list) = 
  print_endline (Int.to_string (fst (List.hd_exn trace)));
  Sequence.range ~start:`inclusive ~stop: `inclusive ~stride:40 20 (fst (List.hd_exn trace))
  |> Sequence.fold ~init: 0 ~f:(fun accum query_cycle ->
      let state = value_at_cycle trace query_cycle in
      print_endline ("query cycle: " ^ (Int.to_string query_cycle) ^ " " ^ (Int.to_string state));
