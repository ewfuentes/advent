  ...
  let rec update_helper cost_map : int array array =
    ...
    Array.iteri grid ~f:(fun row_idx row ->
        Array.iteri row ~f:(fun col_idx item ->
            let min_height_delta, max_height_delta = match travel_dir with
            | Uphill -> Int.min_value, 1
            | Downhill -> -1, Int.max_value in
            (* check neighbors *)
            (* Check above *)
            (if row_idx > 0 then
               ...
               in
               if
                 Int.between ~low:min_height_delta ~high:max_height_delta height_delta
